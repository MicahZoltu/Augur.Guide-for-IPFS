...
All [REP]({{glossary_reputation_token}}) in the [parent universe]({{glossary_parent_universe}}) which are *not* staked on an [outcome]({{glossary_outcome}}) of [forking markets]({{glossary_forking_market}}) can be migrated to one of the [child universes]({{glossary_child_universe}}) by their owner. The migration of REP must be done during the [forking period]({{glossary_forking_period}}) which lasts 60 days after the [fork]({{glossary_fork}}) starts. After the forking period, the REP in the parent universe may not be migrated to any child universe permanently (See [to-do's]({{url_to_dos}}) for details).
